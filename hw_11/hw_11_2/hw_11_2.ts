...
const divRecipes: HTMLDivElement = document.getElementById(
  "wrap"
) as HTMLDivElement;
const listRecipes: HTMLUListElement = document.createElement("ul");
listRecipes.classList.add("list_recipes");
divRecipes.appendChild(listRecipes);

fetch("https://dummyjson.com/recipes ")
  .then((value) => value.json())
  .then((recipesObj: IRecipesResponse) => {
    console.log(" recipesObj:", recipesObj);
    const { recipes } = recipesObj;
    console.log(" resipes:", recipes);

    for (const recipe of recipes) {
      const itemRecipe: HTMLLIElement = document.createElement("li");
      ...
      const nameRecipe: HTMLHeadingElement = document.createElement("h3");
      nameRecipe.classList.add("title");
      nameRecipe.innerText = recipe.name;

      const cuisine: HTMLSpanElement = document.createElement("span");
      cuisine.classList.add("cuisine");
      cuisine.innerText = `(${recipe.cuisine})`;

      nameRecipe.appendChild(cuisine);

      const tagsRecipe: string[] = recipe.tags;
      const itemsTags: HTMLParagraphElement = document.createElement("p");
      itemsTags.classList.add("tags");
      itemsTags.innerText = `Tags: ${tagsRecipe.join(", ")}`;

      const divRating: HTMLDivElement = document.createElement("div");
      ...
      function renderStars(rating: number, container: HTMLElement) {
        container.innerHTML = "";
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          const fillAmount: number = rating - (i - 1);
      ...
      if (!document.getElementById("starGradientDefs")) {
        const defs: SVGSVGElement = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "svg"
        );
  ...
  `;
      ...
      const review: HTMLParagraphElement = document.createElement("p");
      ...
      const imgRecipe: HTMLImageElement = document.createElement("img");
      imgRecipe.src = recipe.image;
      imgRecipe.classList.add("recipe_img");

      const rightBlock: HTMLDivElement = document.createElement("div");
      rightBlock.classList.add("right_block");

      const rightUpBlock: HTMLDivElement = document.createElement("div");
      rightUpBlock.classList.add("rightUp_block");

      const ingredientsBlock: HTMLDivElement = document.createElement("div");
      ...
      const ingrList: HTMLOListElement = document.createElement("ol");

      ingredientsBlock.append(ingredientsTitle, ingrList);

      ingrList.classList.add("ingredients_list");
      for (const ingredient of ingredientsList) {
        const itemIngredient: HTMLLIElement = document.createElement("li");
        itemIngredient.innerText = ingredient;
        ingrList.appendChild(itemIngredient);
      }

      const divTime: HTMLDivElement = document.createElement("div");
      ...
      const timeImg: HTMLImageElement = document.createElement("img");
      timeImg.src = "./images/timer.png";

      const calor: HTMLParagraphElement = document.createElement("p");
      ...
      const instructionsRecipe: string[] = recipe.instructions;
      const instruction: HTMLParagraphElement = document.createElement("p");
      instruction.classList.add("instruction");
      instruction.innerText = `Instruction: ${instructionsRecipe.join(" ")}`;

      const servings: HTMLParagraphElement = document.createElement("p");
      servings.classList.add("servings");
      servings.innerText = `Servings: ${recipe.servings}`;

      const difficulty: string = recipe.difficulty;
      const difficultyLevel: HTMLParagraphElement = document.createElement("p");
      ...
      const meal: string[] = recipe.mealType;
      const mealTags: HTMLParagraphElement = document.createElement("p");
      ...
      const divFooter: HTMLDivElement = document.createElement("div");
