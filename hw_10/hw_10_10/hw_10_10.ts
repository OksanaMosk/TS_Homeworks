type MoovieType = { id: number; name: string; image: string };

const arrayMovie: MoovieType[] = [
  {
    id: 1,
    name: "The Last of Us",
...
const butPrev: HTMLButtonElement = document.getElementsByTagName("button")[0];
const butNext: HTMLButtonElement = document.getElementsByTagName("button")[1];

let listOfMovie: HTMLUListElement = document.createElement("ul");
listOfMovie.classList.add("items");

let currentPage: number = 0;
let itemsPage: number = 10;
function updateList() {
  let startItem: number = currentPage * itemsPage;
  let endItem: number = startItem + itemsPage;

  let itemsInPage: MoovieType[] = arrayMovie.slice(startItem, endItem);
  ...
  for (let i = 0; i < itemsInPage.length; i++) {
    ...
    let itemName: HTMLHeadingElement = document.createElement("h2");
